...
- **Next:** Milestone 3 — **Workspace Manager**: real `git worktree` per agent, compute the diff on completion (the conductor's review surface), serialized merge, and `cleanup` wiring on terminal transitions. This is what turns the streamed Copilot runs into the "review the diff, merge" product. Then M4 (VS Code cockpit). The superseded Claude-adapter plan (`2026-06-14-maestro-claude-adapter.md`) is retained for its Agent-SDK research and auth finding.
